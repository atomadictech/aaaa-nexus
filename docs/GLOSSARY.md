...
## HELIX Compression
Atomadic's tensor compression system, achieving up to 83% size reduction while preserving data integrity.

## Lean 4
A programming language and interactive theorem prover used to write machine-checked mathematical proofs. Atomadic uses Lean 4 to formally verify all safety claims.

## MCP (Model Context Protocol)
A protocol for connecting AI models to external tools and data sources. Atomadic provides an MCP server that exposes its tools to Claude, Cursor, and other MCP-compatible clients.
...
## RatchetGate
Atomadic's session re-keying protocol. Uses a cryptographic ratchet with a formally proved re-key window to prevent session fixation and replay attacks.
...
## Sybil Attack
An attack where a single entity creates many fake identities to gain disproportionate influence. Atomadic's Topological Identity system uses formally proved depth limits to resist Sybil attacks.

## VeriRand
Atomadic's verified randomness service. Produces quantum-sourced random numbers with cryptographic proofs of fairness and non-tampering.
...
## USDC
USD Coin — a stablecoin pegged 1:1 to the US dollar. Used by Atomadic for x402 payments on Base L2, Polygon, and Solana.
